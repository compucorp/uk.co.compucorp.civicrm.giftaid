...
spl_autoload_register('_civigiftaid_civix_class_loader', TRUE, TRUE);

function _civigiftaid_civix_class_loader($class) {
  if ($class === 'CRM_Civigiftaid_DAO_Base') {
    if (version_compare(CRM_Utils_System::version(), '5.74.beta', '>=')) {
      class_alias('CRM_Core_DAO_Base', 'CRM_Civigiftaid_DAO_Base');
      // ^^ Materialize concrete names -- encourage IDE's to pick up on this association.
    }
    else {
      $realClass = 'CiviMix\\Schema\\Civigiftaid\\DAO';
  ...
  if (strpos($class, 'CiviMix\\Schema\\Civigiftaid\\') === 0) {
 ...
 */
function _civigiftaid_civix_civicrm_enable(): void {
  _civigiftaid_civix_civicrm_config();
  // Based on <compatibility>, this does not currently require mixin/polyfill.php.
}
  ...
  if (empty($path)) {
    ...
    foreach ($menu as $key => &$entry) {
      if ($entry['attributes']['name'] == $first) {
        ...
        $found = _civigiftaid_civix_insert_navigation_menu($entry['child'], implode('/', $path), $item);
      }
    }
    return $found;
  }
